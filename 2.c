...
printf("column = %d; row = %d\n", n, rc);
    ...
    while (error_block < 50/*100*/) {
        ...
        printf(" 2. TXT errorblock = %d num %d \n",error_block,num);
        error1 = 0;
        error2 = 0;
        ...
        fclose(outfp2);*/
        FILE *outfp2; 
        outfp2 = fopen("QAM16_UEP_label2_LEVEL2_totalerror_alpha=1.05.txt","a");
        if(error2!=0)fprintf(outfp2,"num = %d ;SNR = %g ; totalerror1 = %d; totalerror2 = %d\n",num , ebn0s, totalerror1, totalerror2);
        fclose(outfp2);
    }
    ...
    FILE *outfp1; 
        outfp1 = fopen("QAM16_UEP_label2_LEVEL2_coef_alpha=1.05.txt","a");
